...
def audit_timing_uniformity():
    ...
    time_unknown_user = measure_response_time(
        "doesnotexist_987@example.local", "WrongPassword123"
    )
    ...
    if variance > 0.05:
        print(
            "[WARNING] Significant timing variance detected. Consider adding a constant-delay or padding logic to authentication routines."
        )
    else:
        print(
            "[SUCCESS] Response times are uniform. Endpoint is resilient against basic timing side-channel enumeration."
        )
