...
def run_cmd(command: list[str]) -> str:
    ...
    if GITHUB_TOKEN and any(GITHUB_TOKEN in c for c in command):
        clean_cmd = " ".join([c if GITHUB_TOKEN not in c else "[REDACTED_TOKEN]" for c in command])
    else:
...
def main() -> None:
    ...
    if not os.path.exists(".git"):
        ...
        run_cmd(["git", "config", "user.email", f"{GITHUB_USER}@users.noreply.github.com"])
    else:
    ...
    if not status:
        ...
        run_cmd(["git", "commit", "-m", "Automated backend sync from Jupyter environment"])

    # 4. Set the branch name explicitly
    run_cmd(["git", "branch", "-M", BRANCH_NAME])

    # 5. Build authenticated URL and push code
    authenticated_url = f"https://{GITHUB_TOKEN}@github.com/{GITHUB_USER}/{GITHUB_REPO}.git"
