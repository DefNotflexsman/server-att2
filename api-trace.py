...
class RubyTrafficSniffer(http.server.BaseHTTPRequestHandler):
    ...
    def print_request_details(self, method, path, headers, body_data=None):
        """Prints full structural diagnostic metrics of the intercepted
        traffic."""
        ...
        print("\n" + "=" * 60)
        print(
            f"[{time.strftime('%H:%M:%S')}] 🚨 INTERCEPTED REQUEST: {method} -> {parsed_url.path}"
        )
        print("=" * 60)
    ...
    def forward_request(self, method, path, headers, data=None):
        """Forwards traffic seamlessly to the real server and mirrors
        responses."""
        ...
        try:
            if method == "GET":
                ...
                res = requests.post(
                    target_url, headers=clean_headers, data=data, timeout=10
                )
            else:
                res = requests.request(
                    method, target_url, headers=clean_headers, data=data, timeout=10
                )
            ...
            print(
                f"[✓] Forwarded successfully. Backend responded with HTTP {res.status_code}"
            )
    ...
    def do_POST(self):
        """Intercepts, parses, and documents payload-heavy POST
        interactions."""
...
def launch_sniffer():
    ...
    with socketserver.TCPServer(("", PORT), RubyTrafficSniffer) as httpd:
        print("[*] Core API Traffic Sniffer Online.")
