...
class Branch(NonCompletableGithubObject):
    ...
    def edit_protection(
        ...
        if is_defined(checks):
            assert all(
                not isinstance(check, tuple) or list(map(type, check)) == [str, int]
                for check in checks
            ), checks
        ...
        if is_defined(strict) or is_defined(contexts) or is_defined(checks):
            ...
            if is_defined(checks):
                checks_parameters = [
                    {"context": check[0], "app_id": check[1]}
    ...
    def edit_required_status_checks(
        ...
        post_parameters: dict[str, Any] = NotSet.remove_unset_items(
            {"strict": strict, "checks": checks_parameters}
        )
    ...
    def get_allow_deletions(self) -> bool:
        ...
        """
        headers, data = self._requester.requestJsonAndCheck(
            "GET", f"{self.protection_url}/allow_deletions"
        )
        return data["enabled"]
    ...
    def set_allow_deletions(self) -> None:
        ...
        """
        headers, data = self._requester.requestJsonAndCheck(
            "POST", f"{self.protection_url}/allow_deletions"
        )
    ...
    def remove_allow_deletions(self) -> None:
        ...
        """
        headers, data = self._requester.requestJsonAndCheck(
            "DELETE", f"{self.protection_url}/allow_deletions"
        )
