...
class Requester:
    ...
    _frameCount: int
    __connectionClass: type[HTTPRequestsConnectionClass] | type[
    ...
    def __init__(
        ...
        self.__graphql_url = urllib.parse.urlunparse(
            o._replace(path=self.__graphql_prefix)
        )
        ...
        self.__connection: HTTPRequestsConnectionClass | HTTPSRequestsConnectionClass | None = (
            None
        )
    ...
    def withLazy(self, lazy: Opt[bool]) -> Requester:
        ...
        :param lazy: if True, completable objects created from this
            instance are lazy, as well as completable objects created
            from those, and so on.
        :return: new Requester instance if is_defined(lazy) and lazy !=
            self.is_lazy, this instance otherwise
        """
    ...
    def withApiVersion(self, api_version: str | None) -> Requester:
        """Create a new requester instance with identical configuration but the
        given API version setting.

        :param api_version: string, GitHub API version to use (see
            https://docs.github.com/en/rest/about-the-rest- api/api-
            versions). Note that some PyGithub methods might downgrade
            this version if it is not supported by the implementation.
    ...
    def paths_of_dict(cls, d: dict) -> dict:
        return {
            key: cls.paths_of_dict(val) if isinstance(val, dict) else None
    ...
    def data_as_class(
        ...
        for item in data_path:
            if item not in data:
                raise RuntimeError(
                    f"GraphQL path {data_path} not found in data: {self.paths_of_dict(data)}"
                )
            data = data[item]
        if klass.is_rest():
    ...
    def graphql_query(
        ...
        response_headers, data = self.requestJsonAndCheck(
            "POST", self.graphql_url, input=input_
        )
        if "errors" in data:
            if len(data["errors"]) == 1:
                ...
                if error.get("type") == "NOT_FOUND":
                    raise github.UnknownObjectException(
                        404, data, response_headers, error.get("message")
                    )
    ...
    def graphql_query_class(
        ...
        """Queries the GraphQL API, extracts data from a given path, populates
        and returns a PyGithub class instance.
    ...
    def graphql_node(
        ...
        actual_node_type = (
            data.get("data", {}).get("node", {}).get("__typename", node_type)
        )
    ...
    def graphql_node_class(
        ...
        """Fetches a GraphQL node by id in the give schema, and returns a
        PyGithub instance of the given class populated with the retrieved
        properties.
    ...
    def graphql_named_mutation_class(
        ...
        """Send a GraphQL mutation to the GraphQL API, populates and returns a
        ...
        :raises: :class:`GithubException` for error status codes
        """
        headers, data = self.graphql_named_mutation(
            mutation_name, mutation_input, output_schema
        )
    ...
    def __customConnection(
        self, url: str
    ) -> HTTPRequestsConnectionClass | HTTPSRequestsConnectionClass | None:
        cnx: HTTPRequestsConnectionClass | HTTPSRequestsConnectionClass | None = None
    ...
    def createException(
        ...
        if status == 401 and lc_message == "bad credentials":
        ...
        elif status == 403 and lc_message.startswith(
            ...
            "not found" in lc_message or "no object found" in lc_message
        ):
    ...
    def getFile(
        ...
        """GET a file from the server and save it to the given path, which
        includes the filename."""
        _, _, stream_chunk_iterator = self.getStream(
            url, parameters, headers, cnx, chunk_size=chunk_size
        )
    ...
    def requestMultipart(
        ...
        :param input: request body, will be serialized as multipart form
            data
        :returns:``(status, headers, body)``
        """
    ...
    def requestMemoryBlobAndCheck(
        ...
        if isinstance(output, str):
            return self.__postProcess(
                verb, url, *self.__check(status, responseHeaders, output)
            )
        raise ValueError(
            "requestMemoryBlobAndCheck() Expected a str, should never happen"
        )
    ...
    def __requestEncode(
        ...
        self.DEBUG_ON_RESPONSE(
            status, responseHeaders, output if isinstance(output, str) else "stream"
        )
        ...
        try:
            ...
            if status == 202 and (
                ...
                return self.__requestRaw(
                    original_cnx, verb, url, requestHeaders, input, stream=stream
                )
            ...
            if status == 301 and "location" in responseHeaders:
                ...
                return self.__requestRaw(
                    original_cnx, verb, o.path, requestHeaders, input, stream=stream
                )
            if status == 302 and follow_302_redirect and "location" in responseHeaders:
                ...
                if self._logger.isEnabledFor(logging.DEBUG):
                    self._logger.debug(
                        f"Following Github server redirection (302) from {url} to {o.path}"
                    )
                # remove auth to not leak authentication to redirection location
                if o.hostname != self.__hostname:
                    requestHeaders = {
                        k: v for k, v in requestHeaders.items() if k != "Authorization"
                    }
                return self.__requestRaw(
                    cnx,
                    verb,
    ...
    def __createConnection(
        ...
        if (
            ...
            and hostname == self.__connection.host
        ):
